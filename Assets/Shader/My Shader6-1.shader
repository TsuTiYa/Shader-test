	...
	SubShader
	{
		...
		{  
			...
			CGPROGRAM
			 ...
			 struct geoout
			 {
				 //SV_POSITION = 各ステージごとに変換され使われるオブジェクトの頂点位置
			...
			//入力された頂点情報と法線ベクトルを元に、変形処理及び出力・構造体への代入を行う
			...
			void geo(triangle vertout In[3],inout TriangleStream<geoout>TriStream)
			{
				...
				pos[1] = setPos(0,0,In[0].pos,In[1].pos,In[2].pos);
				pos[2] = setPos(0,0,In[0].pos,In[1].pos,In[2].pos);
				pos[3] = setPos(0,0,In[0].pos,In[1].pos,In[2].pos);
				pos[4] = setPos(0,0,In[0].pos,In[1].pos,In[2].pos);
				pos[5] = setPos(0,0,In[0].pos,In[1].pos,In[2].pos);
			...
			fixed4 frag(geoout In):COLOR 
			{
				...
				float4 color = UNITY_LIGHTMODEL_AMBIENT + (_Diffuse * _LightColor0 * diffuse + _LightColor0 * half4(1,1,1,1) * specular ) * atten;
