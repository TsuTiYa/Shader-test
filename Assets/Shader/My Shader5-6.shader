	...
	SubShader
	{
		Pass
		{
			...
			vertout vert(appdata_full v)
			{
				...
				OUT.viewDir = normalize(ObjSpaceLightDir(v.vertex));
			...
			fixed4 frag(vertout In):COLOR
			{
				...
				float4 color = (UNITY_LIGHTMODEL_AMBIENT + (_Diffuse * _LightColor0 * diffuse + 
															_LightColor0 * half4(1,1,1,1) * specular) * atten) + Emission;
				return color;
			}
			ENDCG
		}
